...
namespace Escalyn.Api.Services.AuthServices
{
    public class NhostAuthService : INhostAuthService
    {
        ...
        // The string values Nhost / Hasura expect in the JWT roles claim.
        ...
        public async Task<AuthResponse> SignInAsync(SignInRequest request)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(nhostUser?.FirstName))
                        user.FirstName = nhostUser.FirstName;
                    if (!string.IsNullOrEmpty(nhostUser?.MiddleName))
                        user.MiddleName = nhostUser.MiddleName;
                    if (!string.IsNullOrEmpty(nhostUser?.LastName))
                        user.LastName = nhostUser.LastName;
                    if (!string.IsNullOrEmpty(nhostUser?.PhoneNumber))
                        user.PhoneNumber = nhostUser.PhoneNumber;
                    if (!string.IsNullOrEmpty(nhostUser?.Language))
                        user.Language = nhostUser.Language;
        ...
        {
            // TODO: call the Nhost token-verification endpoint instead of a local string check.
            try
            {
